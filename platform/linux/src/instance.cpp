...
Response Instance::onRequest(std::string url_str) {
  ...
  if (url.path == "/platform") {
    responseData = std::vector<unsigned char>((unsigned char *)platform.data(),
    ...
    int payloadBodySize = combineBuffers(pathnameSizeBuffer, 4, pathnameData,
                                         pathnameSize, payloadBody);

    char *payload = new char[2 + payloadBodySize];
    int payloadSize =
        combineBuffers(payloadHeader, 2, payloadBody, payloadBodySize, payload);
...
std::string Instance::onBridge(std::string payload) {
  ...
  int responseWithIdSize = combineBuffers(reqId, 4, (char *)libResponse.data(),
                                          libResponse.size(), responseWithId);
...
std::vector<unsigned char> Instance::callLib(char *data, int size) {
  ...
  int payloadSize = combineBuffers(tmpHeader, headerSize, data, size, payload);
